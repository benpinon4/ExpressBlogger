...
router.get("/", function (req, res, next) {
  res.json({
    success: true,
    route: "blogs",
    message: "hello from the blogs deault route",
  });
});
...
router.get("/single/:blogTitleToGet", function (req, res, next) {
  ...
  const blogTitleToGetIndex = sampleBlogs.findIndex((blog) => {
    return blogTitleToGet === blog.title;
  });
  const titleToGet = sampleBlogs[blogTitleToGetIndex];
  console.log("baLLS" + titleToGet);
  if (blogTitleToGetIndex < 0) {
    res.json({
      sucess: false,
      foundBlog: "Please enter a valid title",
    });
  } else {
    console.log(blogTitleToGetIndex);
    res.json({
      success: true,
      foundBlog: sampleBlogs[blogTitleToGetIndex],
    });
  }
});
...
router.delete("/single/:blogTitleToDelete", function (req, res, next) {
  ...
  const blogTitleToDeleteIndex = sampleBlogs.findIndex((blog) => {
    return blogTitleToDelete === blog.title;
  });

  const deletedTitle = sampleBlogs[blogTitleToDeleteIndex];

  console.log(deletedTitle);

  if (blogTitleToDeleteIndex < 0) {
    res.json({
      success: false,
      deletedBlog: "please enter a valid title",
    });
  } else {
    sampleBlogs.splice(blogTitleToDeleteIndex, 1);

    res.json({
      success: true,
      deletedBlog: deletedTitle,
      blogs: sampleBlogs,
    });
  }
});
